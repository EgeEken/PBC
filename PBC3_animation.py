...
def _make_frame(canvas, color_space, patch_info, separated_channels, title, target=None, show_errors=False, output_size=(3840, 2160)):
    ...
    base_px = max(38, round(frame_h * 0.046))
    min_px = max(28, round(frame_h * 0.032))
    margin = max(18, round(frame_h * 0.020))
    gap = max(8, round(frame_h * 0.010))

    parts = title.split(" | ")
    detail_at = next((i for i, p in enumerate(parts) if p.startswith("ch=")), len(parts))
    title_lines = [" | ".join(parts[:detail_at])]
    if detail_at < len(parts):
        title_lines.append(" | ".join(parts[detail_at:]))

    max_width = frame_w - 2 * margin
    font_px = base_px
    title_font = _font(font_px)
    while font_px > min_px and any(draw.textlength(line, font=title_font) > max_width for line in title_lines):
        font_px -= 2
        title_font = _font(font_px)

    line_gap = max(6, round(font_px * 0.22))
    line_heights = [
        draw.textbbox((0, 0), line, font=title_font)[3] - draw.textbbox((0, 0), line, font=title_font)[1]
        for line in title_lines
    ]
    text_h = sum(line_heights) + line_gap * (len(title_lines) - 1)
    title_h = text_h + max(18, round(font_px * 0.70))

    y = max(8, (title_h - text_h) // 2)
...
def animate_pbc3(
    ...
    for i in range(1, limit + 1):
        if warmup_on and (i - 1) == warmup_split:
            canvas = PBC3._resize_canvas(canvas, warm_w, warm_h)
            if show_errors:
                target = _working_target(original_image, color_space, warm_w, warm_h)
        previous_bytes = current_bytes

        channel, x, y, pw, ph, cell_size, values, bitcount = PBC3._read_patch(br, channel_bits, positive_bias)
        current_bytes = br.i
        current_kb = current_bytes / 1024
        ...
        if now_mse is not None:
            parts.append(f"(-{prev_mse - now_mse:.1f} MSE)")
        parts.append(f"ch={channel} box=({x},{y},{pw},{ph})")
        parts.append(f"cell={cell_size} bitcount={bitcount}")
        title = " | ".join(parts)
        frames.append(_make_frame(
            canvas,
            color_space,
