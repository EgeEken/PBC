...
def residual_sum_error(target, canvas, channel):
    return float(np.sum(np.abs(target[:, :, channel] - np.clip(canvas[:, :, channel], 0, 255))))


def extract_global_features(target, canvas, channel, step=0, patch_count=1):
...
def extract_features(target, canvas, box, step, q, image_w, image_h, patch_count, n_channels):
    ...
    flat = res.ravel(); n = max(1, flat.size); absflat = np.abs(flat)
    ...
    return np.array([
        bw / image_w, bh / image_h, (bw * bh) / (image_w * image_h), np.log((bw + 1e-6) / (bh + 1e-6)),
        c / max(1, n_channels - 1), step / max(1, patch_count), q,
        float(flat.mean()), float(flat.std()), float(absflat.mean()), float(np.sqrt((flat ** 2).mean())),
        float(flat.min()), float(flat.max()), p05, p25, p50, p75, p95,
        float((flat > 0).mean()), float((flat < 0).mean()), float((absflat < 2).mean()), uniq,
        dx, dy, dx + dy, lapx + lapy,
...
def extract_cheap(names, target, canvas, box, step, q, image_w, image_h, patch_count, n_channels):
    ...
    cv = canvas[y:y + bh, x:x + bw, c].astype(np.float64)
    res = t - cv
    flat = res.ravel(); absflat = np.abs(flat)
    d = {
